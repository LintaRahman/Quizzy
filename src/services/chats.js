...
async function loadMessages() {
  if (database.length === 0) {
    ...
    const interviewPrompt = `
        ...
        You are interviewing ${name} who is a ${student ? "student" : "working professional"} 
        with ${years_of_experience} years of experience. The candidate is applying for the job title ${job}, 
        and they provided the following job description: ${job_description}. 
        As Quizzy, your role is to ask relevant questions ${typeOfQ} to assess the candidate's suitability 
        for the ${job} position. Ask interview questions maximum related to their experience, skills, 
        and how they handle specific scenarios. Begin the interview by saying ' Thank you for coming ... ' 
        and only end the interview by saying 'Thank you for attending this interview ...'. 
        Feel free to inquire about their achievements, challenges faced, and how they approach problem-solving 
        but don't ask too long questions. Ask 6 questions, one at a time. Begin the interview by greeting 
        the candidate and proceed with your questions one at a time. Do the interview in English.
...
export async function sendMsgToOpenAI(message) {
  ...
  localStorage.setItem(
    "EndOfInterview",
    botMsg.includes("Thank you for attending")
  );
...
export function downloadTranscript() {
  ...
  const filteredDatabase = replacedDatabase.filter(
